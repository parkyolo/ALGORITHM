...
### 문제풀이
- BFS로 풀이
- ```board[y][x]```가 벽(```#```)이 아니면 BFS 탐색
- ```board[y][x]```를 기준으로 상하좌우를 탐색하면서 벽(```#```)이 아닐때 ```queue```에 넣고 ```'v'```나 ```'k'```이면 ```wolves```나 ```sheep``` 증가
- ```sheep```이 ```wolves```보다 많으면 ```survived_sheep += sheep```
- 아니라면 ```survived_wolves += wolves```