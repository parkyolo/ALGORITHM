...
### 풀이
- ```점수 합의 최솟값 중 최대인 값```을 기준으로 이분 탐색
- 0부터 전체 점수를 합한 값까지를 구간으로 탐색을 시작함
- 0~N번째 값을 ```target```에서 빼면서, ```target```이 0보다 작거나 같아지면 그룹의 수를 count
- ```K```개 이상의 그룹이 생성되면 기준 점수를 높여서 그룹의 수가 더 적어지게 함
- ```K```개보다 적은 그룹이 생성되면 기준 점수를 낮춰서 그룹의 수가 더 많아지게 함